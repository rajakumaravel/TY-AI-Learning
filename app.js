...
function activityHTML(s){const a=s.activity,v=state.activity[s.id]||{};if(a.kind==='quiz')return `<div class="quiz-list">${a.items.map((it,i)=>`<label><span>${esc(it[0])}</span><select data-i="${i}"><option value="">Choose…</option>${a.options.map(o=>`<option value="${esc(o)}" ${v[i]===o?'selected':''}>${esc(o)}</option>`).join('')}</select></label>`).join('')}</div>`;
