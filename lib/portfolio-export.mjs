...
function doc(title,bodyHTML){
  return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(title)}</title><style>${STYLE}</style></head><body>${bodyHTML}</body></html>`;
}

function projectHTML(p){
  if(!p)return '';
  const items=Array.isArray(p.deliverables)?p.deliverables:[];
  const rows=items.map(d=>Array.isArray(d)?[String(d[0]||''),String(d[1]||'')]:['',String(d||'')]).filter(r=>r[0]||r[1]);
  if(!rows.length&&!p.finalRecommendation)return '';
  return `<div class="evidence"><h3>Project${p.status?` · ${esc(p.status)}`:''}</h3>${rows.map(([l,v])=>`<p class="evidence-item">${l?`<b>${esc(l)}:</b> `:''}${esc(v)}</p>`).join('')}${p.finalRecommendation?`<p class="evidence-item"><b>Final recommendation:</b> ${esc(p.finalRecommendation)}</p>`:''}</div>`;
}

function assessmentLine(a){
  if(!a||!(a.level||a.teacherLevel||a.suggestedLevel))return '';
