...
- Automated acceptance (2026-09-11): `tests/acceptance/cutover-acceptance.mjs` passed 27/27 checks against Preview deployment `2836a238` (commit `1563d39`) using three throwaway password-auth users created and deleted through the Supabase admin API. Evidenced: unauthenticated 401s; server-side progress persistence and read-back; project save/submit/review round-trip with exact submitted snapshot visible to admin; RLS denies cross-learner reads of `student_projects`, `learner_progress` and `learners`, and denies direct browser writes; non-admin callers receive 403 on every admin route; app-metadata `role=admin` is honoured.
- Still pending manual validation: real Google OAuth sign-in in a browser; the `ADMIN_EMAILS` allowlist path with the real teacher account; Chapter 1→2 capstone gate through the UI; cross-device persistence on two physical devices; and the exact Production deployment (none exists yet).
