export const CAPSTONES = {
  ...
  block3: {
    id: 'block3-capstone',
    title: 'Data Card review',
    brief: 'A school library app logs every search a student types, every book they borrow and how long they keep it. The developers have added a column called reading_level that is inferred from borrow history and shown to teachers, and they want to use the search log to recommend books. Review the dataset like a junior data analyst before it is used.',
    prompts: [
      'Sort the fields into volunteered, observed and inferred. Which ones are sensitive or unnecessary for recommending books, and what should be removed or anonymised?',
      'The inferred reading_level column is shown to teachers. What could go wrong for a student, and what evidence would you want before trusting it?',
      'Write the key lines of a Responsible Data Card for this app: what the data can be used for, what it must not be used for, and who should check it.'
    ]
  }
};
...
export function assessChapterCapstone({blockId,answers={}}){
  ...
  const hasConcept=/\b(input|data|pattern|predict|prediction|recommend|accuracy|confusion|shortcut|training|test|model|output|missing|duplicate|inconsistent|imbalance|sensitive|inferred|volunteered|observed)\b/i.test(combined);
  const hasEvidence=new RegExp('\\b(because|evidence|result|8|80|dark|light|background|stakeholder|benefit|risk|example'+(blockId==='block3'?'|column|row|field|search|borrow|library|reading':'')+')\\b','i').test(combined);
  const hasReason=/\b(because|therefore|so|this shows|which means|however|depends|if|why)\b/i.test(combined);
  const hasAction=/\b(human|check|review|retrain|retest|vary|balance|change|compare|oversight|decide|remove|collect|consent|anonymise|anonymize|flag|fix)\b/i.test(combined);
