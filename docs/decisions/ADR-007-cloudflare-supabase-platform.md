...
- Hardening (Phase 2, first slice): chapter unlock was evaluated client-side from `state.chapterAssessments`, which the learner's own progress `PUT` could set. The API now overwrites `chapterAssessments` on every state read and write with rows from `chapter_assessments`, and returns 409 for a chapter capstone, project save or project submit when the previous chapter is not qualified. Covered by `npm run acceptance`.
...
## Validation record — 2026-09-14 (Production release)

- Production was deployed for the first time. Release run `34835574318` deployed commit `64fad52c1c78ede32a25e6471b01f2ced8fbbeff` (the PR #24 merge on `main`) as Cloudflare Pages deployment `228b8cd3`, branch `main`. Immediately before the release, `https://ty-ai-learning.pages.dev` returned 404 and `wrangler pages deployment list` showed zero Production deployments.
- Release 2026-09-22 (`bab0193`, ADR-009 open chapter access): pre-release gate `ALL SUITES GREEN` on preview `01c2e87a` — API 94/94, browser 233/233, walkthrough 258/258. Deployed by CLI as Production `9c7118a9`; no migration ran (`Remote database is up to date.`) and row counts were unchanged either side. Production was verified read-only, because the acceptance suites write throwaway auth users to the live database.
- Pre-release gate: `scripts/verify-preview.sh main` reported `ALL SUITES GREEN` against preview deployment `2a80d962` — `cutover-acceptance.mjs` 94/94, `ui-acceptance.mjs` 235/235, `live-walkthrough.mjs` 258/258.
- Production acceptance against `https://ty-ai-learning.pages.dev`: `cutover-acceptance.mjs` 94/94, `ui-acceptance.mjs` 235/235, `live-walkthrough.mjs` 258/258 — the same counts as Preview. As on Preview, these suites create and delete throwaway auth users in the live Supabase project, which is the same project the pilot uses.
- Database unchanged. Both the dry-run and the apply reported `Remote database is up to date.`; `20260910115137` and `20260910120000` remain the only migrations, so this release altered no schema.
- Production runtime bindings were verified by CLI and API rather than operator report, closing the gap left open on 2026-09-11: `SUPABASE_SERVICE_ROLE_KEY` present as an encrypted secret, and `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `ADMIN_EMAILS` present in `deployment_configs.production.env_vars`, with `production_branch=main`. Presence only — the values were not compared against Preview.
- Three dispatches failed first (`34834129171`, `34834429193`, `34834579198`), all at the migration dry-run, none reaching apply, build or deploy. The `SUPABASE_DB_URL` secret in the GitHub `production` environment held a stale password; the runbook had recorded that secret as good. The password was reset in the Supabase dashboard and the connection string verified with `psql` before the final dispatch. Note for future releases: `supabase migration list --linked` succeeding locally proves nothing about this secret, since the CLI authenticates from the keychain session instead.
- Supabase Auth Site URL and redirect allow-list were updated by the operator to cover `https://ty-ai-learning.pages.dev`. This could not be read back — no management-API token is configured here — so it rests on operator report until the manual sign-in check.
- **Outstanding:** the operator has not yet confirmed by hand that Google sign-in works on the Production URL and that `/admin` opens for the real teacher account in `ADMIN_EMAILS`. Until that is done, the ADR-007 exit criteria "student Google sign-in works" and "admin routes reject non-admin users" are evidenced on Preview and by automated Production suites, but not by a real Google account against Production.
