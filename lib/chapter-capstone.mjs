...
export function assessChapterCapstone({blockId,answers={}}){
  ...
  const hasConcept=/\b(input|data|pattern|predict|prediction|recommend|accuracy|confusion|shortcut|training|test|model|output)\b/i.test(combined);
  const hasEvidence=/\b(because|evidence|result|8|80|dark|light|background|stakeholder|benefit|risk|example)\b/i.test(combined);
  const hasReason=/\b(because|therefore|so|this shows|which means|however|depends|if|why)\b/i.test(combined);
  const hasAction=/\b(human|check|review|retrain|retest|vary|balance|change|compare|oversight|decide)\b/i.test(combined);
